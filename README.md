...
### Step-by-Step Overview:
Step 1: Load Primer Panel CSV
Reads in the Primer3-designed SNP panel (gtseq_panel.csv)

Step 2: Load SNP Metadata
Parses snp_metadata.tsv for chromosome, position, and minor allele frequency (MAF)

Step 3: Load BED File
Reads genomic start/end coordinates from the BED file used to create the input FASTA

Step 4: Combine Data
For each SNP, merges data from the three sources into a single annotated row

Step 5: Write Final Annotated Panel
Outputs the final GT-seq panel 
